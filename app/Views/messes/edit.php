...
$nameValue = $old['name'] ?? $mess['name'] ?? '';
$locationValue = $old['location'] ?? $mess['location'] ?? '';
$rentValue = $old['rent'] ?? $mess['rent'] ?? '';
$descriptionValue = $old['description'] ?? $mess['description'] ?? '';
$imageValue = $mess['image'] ?? '';
?>
<!doctype html>
<html lang="en">
<head>
    ...
    <title>Edit Mess | The Mess Hub</title>
...
<body class="min-h-screen bg-slate-50 text-slate-900">
    <div class="min-h-screen font-['Space_Grotesk']">
        ...
        <main class="mx-auto max-w-2xl px-6 py-10">
            <header class="mb-8">
                <p class="text-xs font-semibold uppercase tracking-[0.3em] text-blue-600">Edit</p>
                <h1 class="mt-2 text-3xl font-semibold">Edit Mess</h1>
            </header>

            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <?php if ($updated): ?>
                    <div class="mb-4 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">Mess updated successfully.</div>
                <?php endif; ?>
                ...
                <form class="space-y-6" method="post" action="/messes/update" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200" id="name" name="name" type="text" value="<?php echo htmlspecialchars((string) $nameValue, ENT_QUOTES, 'UTF-8'); ?>" required>
                    ...
                    <div>
                        ...
                        <input class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200" id="location" name="location" type="text" value="<?php echo htmlspecialchars((string) $locationValue, ENT_QUOTES, 'UTF-8'); ?>" required>
                    ...
                    <div>
                        ...
                        <input class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200" id="rent" name="rent" type="number" step="0.01" min="0" value="<?php echo htmlspecialchars((string) $rentValue, ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <div>
                        ...
                        <textarea class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200" id="description" name="description" rows="4"><?php echo htmlspecialchars((string) $descriptionValue, ENT_QUOTES, 'UTF-8'); ?></textarea>
                    ...
                    <div>
                        ...
                        <div class="mt-2 relative" id="drop-zone">
                            ...
                            <div id="upload-placeholder" class="<?php echo $imageValue ? 'hidden' : ''; ?> flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center transition-all hover:border-blue-400 hover:bg-blue-50/30">
                            ...
                            <div id="preview-container" class="<?php echo $imageValue ? '' : 'hidden'; ?> rounded-xl overflow-hidden border border-slate-200 shadow-sm">
                                <img id="image-preview" class="w-full h-48 object-cover" src="<?php echo htmlspecialchars($imageValue, ENT_QUOTES, 'UTF-8'); ?>" alt="Mess image preview">
                                <div class="flex items-center justify-between bg-slate-50 px-4 py-2">
                                    <span id="file-name" class="text-xs text-slate-600 truncate"><?php echo $imageValue ? 'Current image' : ''; ?></span>
                    ...
                    <button class="w-full rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 transition" type="submit">Update Mess</button>
                </form>
            </div>
        </main>
    </div>
